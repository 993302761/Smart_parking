...
class CardPredictor:
    ...
    def predict(self, car_pic, resize_rate=1):
        ...
        if len(predict_result) != 0:
            return predict_result, roi, card_color, 1  # 识别到的字符、定位的车牌图像、车牌颜色
        else:
...
def Action(Basepath, frame):
    ...
    if p0 == 0:
        # 定一个字典类型
        mapA = {"License_plate": "null", "color": "null", "time": nowt, "id": "1001", "isusing": True}
        # 把字典转成字符串
        ...
        # 定一个字典类型
        mapA = {"License_plate": s0, "color": color0, "time": nowt, "id": "1001" ,"isusing":False}
        # 把字典转成字符串
        str0 = json.dumps(mapA)
        # 这里得给设置一个请求格式，不然会返回415
        # header_dict = {'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; Trident/7.0; rv:11.0) like Gecko',"Content-Type": "application/json"}
...
if __name__ == '__main__':
    # 设置要访问的地址
    url = 'http://localhost:8080/pythonToJavaTest'
    c = CardPredictor()
    c.train_svm()
    show()
